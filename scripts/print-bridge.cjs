...
try{labelHtml=decodeJob();document.getElementById("preview").srcdoc=labelHtml.replace(/<script\\b[^>]*>[\\s\\S]*?<\\/script\\s*>/gi,"");statusBox.textContent="PC encontrado. La etiqueta está lista.";statusBox.classList.add("ok");printButton.disabled=false}catch(error){statusBox.textContent=error.message;statusBox.classList.add("error")}
