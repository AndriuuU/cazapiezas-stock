...
const EMPTY_FILTERS: Filters = {
  ...
  publicado_online: "", ubicacion: "", tipo_pieza: "", venta_desde: "", venta_hasta: "",
};
...
export default function WarehouseList({ initialView = "almacen", initialType = "" }: { initialView?: ListView; initialType?: "CAT" | "IAM" | "" }) {
  ...
  const [filters, setFilters] = useState<Filters>({ ...EMPTY_FILTERS, tipo_pieza: initialType });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (view === "vendidas") params.set("iva", String(vatRate));
  ...
  }, [filters, page, pageSize, sort, vatRate, view]);
    ...
    <main className="warehouse-list-main min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
      ...
      <div className="mx-auto max-w-[1500px] space-y-4 px-4 py-4 sm:space-y-5 sm:px-6 sm:py-6">
        ...
        {view === "vendidas" && <SalesSummaryPanel
          ...
          onVatRate={(value) => { setVatRate(value); setPage(1); }}
          onClearDates={() => { setFilters((current) => ({ ...current, venta_desde: "", venta_hasta: "" })); setPage(1); }}
        />}
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-8">
            ...
            <div className={showMobileFilters ? "contents" : "hidden md:contents"}>
              ...
              <FilterSelect value={filters.tipo_pieza} onChange={(value) => updateFilter("tipo_pieza", value)}><option value="">CAT e IAM</option><option value="CAT">Solo CAT</option><option value="IAM">Solo IAM</option></FilterSelect>
              ...
              <div className="bulk-actions-grid">
                <div className="rounded-xl border border-zinc-700 bg-zinc-950/70 p-2.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={() => setConfirmation({ title: `¿Publicar ${selected.size} piezas en Recambio Fácil?`, description: "Antes de enviarlas se comprobarán Nombre de la pieza, Referencia principal de al menos 4 caracteres, Precio, Marca y Modelo. El nombre se enviará en el campo Descripcion de R/F. Se enviarán en lotes de hasta 10 piezas y solo se marcarán Online las confirmadas por Recambio Fácil.", confirmLabel: `Sí, publicar ${selected.size}`, onConfirm: () => publishPieces([...selected]) })} disabled={publishing} title="Requiere nombre, referencia de 4 caracteres, precio, marca y modelo" className="inline-flex min-h-10 min-w-0 items-center justify-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900 px-2 text-xs font-bold text-zinc-200 hover:border-cyan-500/60 hover:bg-zinc-800 disabled:opacity-50">{publishing ? <Loader2 className="shrink-0 animate-spin text-cyan-400" size={16} /> : <CloudUpload className="shrink-0 text-cyan-400" size={16} />}<span>Publicar ({selected.size})</span></button>
                  </div>
                </div>
...
function TypeBadge({ type }: { type: PiezaDesguace["tipo_pieza"] | undefined }) {
  const iam = type === "IAM";
  return <span className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-black ${iam ? "bg-violet-500/20 text-violet-300" : "bg-cyan-500/15 text-cyan-300"}`}>{iam ? "IAM" : "CAT"}</span>;
}
...
function SalesSummaryPanel({
  ...
  const cards = [
    ...
    { label: `IVA incluido · ${vatRate} %`, value: money(summary?.vat), color: "text-amber-300" },
  ...
  return <section className={`rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 transition-opacity sm:p-5 ${loading ? "opacity-60" : ""}`}>
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <label className="text-xs font-bold text-zinc-400"><span className="mb-1 block">Tipo de IVA</span><select aria-label="Tipo de IVA" value={vatRate} onChange={(event) => onVatRate(Number(event.target.value))} className="min-h-10 rounded-xl border border-zinc-700 bg-zinc-950 px-3 text-sm text-white outline-none focus:border-amber-500"><option value={21}>21 % general</option><option value={10}>10 %</option><option value={4}>4 %</option><option value={0}>0 % / exenta</option></select></label>
        {(from || to) && <button type="button" onClick={onClearDates} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-zinc-700 px-3 text-xs font-bold text-zinc-300 hover:bg-zinc-800"><CalendarRange size={15} /> Todas las fechas</button>}
      </div>
    </div>
