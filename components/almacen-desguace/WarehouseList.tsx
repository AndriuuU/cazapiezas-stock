...
function PieceCard({ piece, selected, expanded, onToggle, onPanel, onLocate, onDrawer, onPhotos, onAction }: PieceItemProps) {
  ...
  return <article className={`p-2.5 ${selected ? "bg-amber-500/5" : ""}`}>
    ...
    <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 rounded-lg bg-zinc-950/60 px-2.5 py-2 text-[11px]"><div><span className="mb-1 block text-zinc-600">Ubicación</span><WarehouseLocationLink location={piece.ubicacion} compact /></div><div><span className="text-zinc-600">Estado pieza</span><p className="truncate text-zinc-300">{piece.estado_pieza || "Sin estado"}</p></div><div className="col-span-2"><span className="text-zinc-600">Proceso</span><p className="truncate text-zinc-400">{piece.estado_proceso}</p></div></div>
