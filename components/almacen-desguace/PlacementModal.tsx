...
export default function PlacementModal({ piece, onClose, onPlaced }: { piece: PiezaDesguace; onClose: () => void; onPlaced: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [response, locationsResponse] = await Promise.all([
        fetch(`/api/almacen-desguace/${piece.id}/ubicar`),
        fetch("/api/almacen-desguace/ubicaciones/disponibles"),
      ]);
      ...
      if (!locationsResponse.ok) throw new Error(locationsBody.error || "No se pudieron cargar los huecos disponibles.");
      setAvailableLocations(locationsBody);
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 p-4 backdrop-blur-sm md:items-center"><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-800 bg-zinc-900 p-4"><div><p className="font-mono text-xs font-bold text-amber-300">{piece.codigo_interno}</p><h2 className="text-xl font-bold text-white">Colocar {piece.nombre_pieza || "pieza"}</h2></div><button onClick={onClose} className="rounded-lg p-2 text-zinc-400 hover:bg-zinc-800"><X /></button></div><div className="space-y-4 p-5">
    {loading ? <div className="flex justify-center gap-2 py-12 text-zinc-400"><Loader2 className="animate-spin" /> Buscando el mejor hueco...</div> : <>
      ...
      {mode === "alternative" && <section className="space-y-4 rounded-2xl border border-amber-500/30 bg-zinc-950 p-4"><div><h3 className="font-bold text-white">Elige dónde la has colocado</h3><p className="text-sm text-zinc-500">Solo aparecen estanterías configuradas y huecos que están libres ahora mismo.</p></div><AvailableLocationPicker locations={availableLocations} value={alternative} onChange={setAlternative} /><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Motivo opcional: no cabía en la recomendada, mejor acceso..." rows={2} className="w-full rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-3 text-white focus:border-amber-500 focus:outline-none" /><div className="flex flex-wrap justify-end gap-2"><button onClick={() => setMode("suggestion")} className="px-3 py-2 text-zinc-400">Volver</button><button disabled={saving || !alternative} onClick={() => void confirm("colocada_alternativa")} className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 font-bold text-zinc-950 disabled:opacity-50">{saving && <Loader2 size={16} className="animate-spin" />} Guardar en {alternative || "un hueco"}</button></div></section>}
