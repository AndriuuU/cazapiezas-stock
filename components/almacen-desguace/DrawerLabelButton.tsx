...
export default function DrawerLabelButton({ drawer }: { drawer: CajonDesguace }) {
  ...
  return <div className="flex flex-wrap gap-2"><button onClick={() => void print("brother")} disabled={busy} className="inline-flex items-center gap-2 rounded-xl border border-cyan-500/30 px-4 py-2.5 font-bold text-cyan-200 hover:bg-cyan-500/10 disabled:opacity-50">{busy ? <Loader2 size={17} className="animate-spin" /> : <Printer size={17} />} Etiqueta QR · 62 × 100</button><button onClick={() => void print("browser")} disabled={busy} className="inline-flex items-center gap-2 rounded-xl border border-zinc-700 px-4 py-2.5 font-bold text-zinc-300 hover:bg-zinc-800 disabled:opacity-50"><Printer size={17} /> PDF / otra impresora</button></div>;
}
