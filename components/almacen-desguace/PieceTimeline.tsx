...
export default function PieceTimeline({ pieza, events, movements }: { pieza: PiezaDesguace; events: EventoAlmacen[]; movements: MovimientoUbicacion[] }) {
  ...
  if (sale) steps.push({ key: "sale", title: "Venta", detail: `${String(sale.valor_nuevo?.empleado || sale.usuario_nombre)} · ${Number(sale.valor_nuevo?.precio_final || 0).toFixed(2)} €`, date: String(sale.valor_nuevo?.fecha_venta || sale.created_at), tone: "bg-emerald-500 text-zinc-950", icon: <ShoppingBag size={18} /> });
