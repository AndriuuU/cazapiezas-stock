...
export default function DrawerDetail({ initialDrawer }: { initialDrawer: CajonDesguace }) {
  ...
  const canManageContents = Boolean(currentUser && (isAdmin || settings.employeesCanManageDrawerContents));
  const canRegisterSales = Boolean(currentUser && (isAdmin || settings.employeesCanRegisterSales));
  ...
  return <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
    ...
    {soldTarget && <SaleModal piece={soldTarget} onClose={() => setSoldTarget(null)} onSold={(message) => { setSoldTarget(null); void reload(message); }} />}
    {canManageContents && scannerOpen && <BarcodeScanner onClose={() => setScannerOpen(false)} onScan={(value) => { setScannerOpen(false); void findPieces(value); }} />}
  </main>;
}
...
function DrawerPieceCard({ piece, admin, canManage, canSell, onSell, onRemove }: { piece: PiezaDesguace; admin: boolean; canManage: boolean; canSell: boolean; onSell: () => void; onRemove: () => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
    ...
    <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-zinc-800 bg-zinc-900/60 px-4 py-3"><div><p className="font-mono text-[10px] text-zinc-500">{piece.codigo_interno}</p><p className="text-xs font-bold text-zinc-300">{piece.estado_proceso}{piece.precio_venta != null ? ` · ${Number(piece.precio_venta).toFixed(2)} €` : ""}</p></div><div className="flex flex-wrap gap-2">{admin && <RecambioFacilLink piece={piece} compact />}<Link href={`/almacen-desguace/${piece.id}`} className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-bold text-zinc-300 hover:border-cyan-500/40 hover:text-cyan-300"><Eye size={15} /> Ver ficha</Link>{canSell && <button onClick={onSell} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/30 px-3 py-2 text-xs font-bold text-emerald-300 hover:bg-emerald-500/10"><ShoppingBag size={15} /> Vendida</button>}{canManage && <button onClick={onRemove} className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/30 px-3 py-2 text-xs font-bold text-red-300 hover:bg-red-500/10"><PackageMinus size={15} /> Retirar</button>}</div></footer>
  </article>;
}
