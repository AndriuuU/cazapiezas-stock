...
import { AlertTriangle, ArrowDownToLine, ArrowLeft, ArrowRight, Box, Camera, CarFront, Clock3, Edit3, Eye, Layers3, Loader2, MapPin, PackageMinus, ScanBarcode, Search, ShoppingBag, Tag, Truck, X } from "lucide-react";
...
export default function DrawerDetail({ initialDrawer }: { initialDrawer: CajonDesguace }) {
  ...
  const [soldTarget, setSoldTarget] = useState<PiezaDesguace | null>(null);
  ...
  async function sellPiece(piece: PiezaDesguace) {
    setBusyId(piece.id); setSaleError(""); setSuccess("");
    try {
      const response = await fetch(`/api/almacen-desguace/${piece.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "vender" }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "No se pudo marcar la pieza como vendida.");
      setSoldTarget(null);
      await reload(`${piece.codigo_interno} marcada como vendida y retirada del cajón.`);
    } catch (caught) {
      setSaleError(caught instanceof Error ? caught.message : "No se pudo marcar la pieza como vendida.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  return <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
    ...
    {soldTarget && <Modal title={`¿Marcar como vendida?`} subtitle={`${soldTarget.codigo_interno} · ${soldTarget.nombre_pieza || "Pieza sin nombre"}`} onClose={() => setSoldTarget(null)}>{saleError && <div role="alert" className="mb-4 flex gap-3 rounded-xl border border-red-400/40 bg-red-950/80 p-4 text-sm text-red-100"><AlertTriangle className="shrink-0 text-red-400" size={20} /><p>{saleError}</p></div>}<div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm leading-6 text-emerald-100"><p className="font-black">La pieza saldrá del cajón y de Piezas almacenadas.</p><p className="mt-1 text-emerald-200/75">Se liberará su espacio en el cajón, aparecerá en Vendidas y dejará de estar marcada como Online en Cazapiezas.</p></div><button onClick={() => void sellPiece(soldTarget)} disabled={busyId === soldTarget.id} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 py-3 font-black text-zinc-950 hover:bg-emerald-400 disabled:opacity-50">{busyId === soldTarget.id ? <Loader2 className="animate-spin" /> : <ShoppingBag />} Sí, marcar como vendida</button></Modal>}
...
function DrawerPieceCard({ piece, onSell, onRemove }: { piece: PiezaDesguace; onSell: () => void; onRemove: () => void }) {
