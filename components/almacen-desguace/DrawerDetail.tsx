...
import { AlertTriangle, ArrowDownToLine, ArrowLeft, ArrowRight, Box, Clock3, Edit3, Layers3, Loader2, MapPin, PackageMinus, Search, Truck, X } from "lucide-react";
...
export default function DrawerDetail({ initialDrawer }: { initialDrawer: CajonDesguace }) {
  ...
  async function findPieces() {
    ...
    try {
      const response = await fetch(`/api/almacen-desguace?q=${encodeURIComponent(search)}&page_size=25`);
  ...
  return <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
    ...
    <div className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.4fr_1fr]"><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-lg font-black text-amber-300">{drawer.codigo}</p><h1 className="text-3xl font-black text-white">{drawer.nombre}</h1><p className="mt-1 text-zinc-500">{drawer.descripcion || "Sin descripción"}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${drawer.lleno ? "bg-red-500/10 text-red-300" : "bg-emerald-500/10 text-emerald-300"}`}>{drawer.lleno ? "LLENO" : "DISPONIBLE"}</span></div><div className="mt-5 flex items-center gap-3 rounded-xl bg-zinc-950 p-4"><MapPin className="text-cyan-400" /><div><p className="text-xs text-zinc-500">Hueco ocupado por este cajón</p><p className="font-mono font-black text-cyan-200">{drawer.ubicacion}</p></div></div></div><div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-5"><p className="text-sm font-bold text-cyan-200">Ocupación del cajón</p><p className="mt-2 text-4xl font-black text-white">{drawer.cantidad_piezas} <span className="text-lg text-zinc-500">/ {drawer.capacidad_maxima}</span></p><div className="mt-4 h-3 overflow-hidden rounded-full bg-zinc-800"><div className={drawer.lleno ? "h-full bg-red-500" : "h-full bg-cyan-500"} style={{ width: `${drawer.porcentaje_ocupacion}%` }} /></div><p className="mt-2 text-sm text-zinc-400">{drawer.disponibles} espacios disponibles</p></div></section>

      <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><div className="mb-4"><h2 className="text-xl font-black text-white">Añadir una pieza</h2><p className="text-sm text-zinc-500">Busca por referencia, nombre, código o matrícula. Si ya está en otro cajón se trasladará.</p></div><div className="flex gap-2"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" size={18} /><input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void findPieces(); }} placeholder="Ej.: 1K0959653, centralita o 1234ABC" className="w-full rounded-xl border border-zinc-700 bg-zinc-950 py-3 pl-10 pr-4 text-white outline-none focus:border-amber-500" /></label><button onClick={() => void findPieces()} disabled={searching || drawer.lleno} className="rounded-xl bg-amber-500 px-5 font-black text-zinc-950 disabled:opacity-40">{searching ? <Loader2 className="animate-spin" /> : "Buscar"}</button></div>{results.length > 0 && <div className="mt-3 divide-y divide-zinc-800 rounded-xl border border-zinc-800 bg-zinc-950">{results.map((piece) => <div key={piece.id} className="flex flex-wrap items-center justify-between gap-3 p-3"><PieceName piece={piece} /><button onClick={() => void changePiece(piece, "add")} disabled={busyId === piece.id || drawer.lleno} className="inline-flex items-center gap-2 rounded-lg bg-cyan-500 px-3 py-2 text-sm font-black text-zinc-950 disabled:opacity-40">{busyId === piece.id ? <Loader2 className="animate-spin" size={16} /> : <ArrowDownToLine size={16} />} {piece.cajon_id ? "Mover aquí" : "Añadir"}</button></div>)}</div>}</section>

      <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-black text-white">Piezas dentro del cajón</h2><p className="text-sm text-zinc-500">{drawer.cantidad_piezas} piezas guardadas</p></div><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" size={16} /><input value={insideQuery} onChange={(e) => setInsideQuery(e.target.value)} placeholder="Filtrar contenido..." className="rounded-xl border border-zinc-700 bg-zinc-950 py-2.5 pl-9 pr-3 text-sm text-white outline-none" /></label></div>{visiblePieces.length ? <div className="divide-y divide-zinc-800 rounded-xl border border-zinc-800 bg-zinc-950">{visiblePieces.map((piece) => <div key={piece.id} className="flex flex-wrap items-center justify-between gap-3 p-3"><PieceName piece={piece} /><div className="flex gap-2"><Link href={`/almacen-desguace/${piece.id}`} className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-bold text-zinc-300">Ver ficha</Link><button onClick={() => { setRemoveTarget(piece); setRemoveError(""); }} className="inline-flex items-center gap-1 rounded-lg border border-red-500/30 px-3 py-2 text-sm font-bold text-red-300"><PackageMinus size={16} /> Retirar</button></div></div>)}</div> : <div className="py-12 text-center text-zinc-500"><Box className="mx-auto mb-2" size={40} /><p>Este cajón todavía no contiene piezas.</p></div>}</section>
