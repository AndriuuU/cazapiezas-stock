...
export default function WarehouseMap({ shelves }: { shelves: EstanteriaPlanoAlmacen[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return shelves.filter((shelf) => {
      ...
      return normalize([shelf.codigo, shelf.nombre, shelf.descripcion, shelf.zona, ...rules, ...pieces].filter(Boolean).join(" ")).includes(term);
    });
  }, [query, shelves, zone]);
  ...
  return <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
    ...
    <div className="mx-auto max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-3"><Link href="/almacen-desguace" className="inline-flex items-center gap-2 text-sm font-semibold text-zinc-400 hover:text-white"><ArrowLeft size={17} /> Volver a las piezas</Link><Link href="/almacen-desguace/estanterias" className="inline-flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-2.5 font-bold text-zinc-200 hover:border-amber-500/40 hover:text-amber-300"><Settings2 size={17} /> Configurar estanterías</Link></div>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-4">
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-xs text-zinc-400"><Legend color="bg-emerald-500" text="Hueco libre" /><Legend color="bg-amber-400" text="Ocupado" /><Legend color="bg-zinc-700" text="No disponible" /><Legend color="bg-red-500" text="Estantería llena o inactiva" /></div>
      </section>
...
function ShelfMapCard({ shelf }: { shelf: EstanteriaPlanoAlmacen }) {
  ...
  return <article className={`overflow-hidden rounded-2xl border bg-zinc-900 ${shelf.llena || !shelf.activa ? "border-red-500/30" : "border-zinc-800"}`}>
...
function Legend({ color, text }: { color: string; text: string }) { return <span className="inline-flex items-center gap-2"><span className={`h-3 w-3 rounded ${color}`} />{text}</span>; }
