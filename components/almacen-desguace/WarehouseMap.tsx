...
export default function WarehouseMap({ shelves }: { shelves: EstanteriaPlanoAlmacen[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return shelves.filter((shelf) => {
      ...
      const drawers = shelf.huecos.flatMap((slot) => slot.cajon ? [slot.cajon.codigo, slot.cajon.nombre] : []);
  ...
  return <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
    ...
    <div className="mx-auto max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-4">
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-xs text-zinc-400"><Legend color="bg-emerald-500" text="Hueco libre" /><Legend color="bg-amber-400" text="Pieza directa" /><Legend color="bg-cyan-400" text="Cajón" /><Legend color="bg-zinc-700" text="No disponible" /><Legend color="bg-red-500" text="Estantería llena o inactiva" /></div>
      </section>
