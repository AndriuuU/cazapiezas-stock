...
export default function PhotoManager({ piezaId, initialPhotos }: { piezaId: number; initialPhotos: FotoDesguace[] }) {
  ...
  async function upload() {
    ...
    const form = new FormData(); Array.from(files).forEach(file => form.append("fotos", file));
    try {
      const response = await fetch(`/api/almacen-desguace/${piezaId}/fotos`, { method: "POST", body: form });
      const data = await response.json(); if (!response.ok) throw new Error(data.error);
      window.location.reload();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "No se pudieron subir."); setBusy(false); }
  }
  ...
  return (
    <section id="fotografias" className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-bold text-white"><Camera size={20} className="text-amber-400" /> Fotografías</h2><p className="text-sm text-zinc-500">La estrella identifica la foto principal.</p></div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 font-bold text-zinc-950 hover:bg-amber-400"><input ref={input} type="file" accept="image/*" multiple className="hidden" onChange={() => void upload()} />{busy ? <Loader2 className="animate-spin" size={17} /> : <Upload size={17} />} Subir fotos</label></div>
      {error && <p className="mb-3 rounded-lg bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}
      {photos.length ? <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-5">{photos.map(photo => <div key={photo.id} className={`group relative overflow-hidden rounded-xl border ${photo.es_principal ? "border-amber-400" : "border-zinc-700"}`}><img src={photo.url_visualizacion} alt="Pieza" className="aspect-square w-full object-cover" /><div className="absolute inset-x-0 bottom-0 flex justify-end gap-1 bg-gradient-to-t from-black/90 p-2 pt-8"><button onClick={() => void copyPublicUrl(photo)} title="Copiar enlace público" className="rounded-lg bg-black/60 p-2 text-cyan-300">{copiedId === photo.id ? <Check size={16} /> : <Copy size={16} />}</button><button disabled={busy || photo.es_principal} onClick={() => void makeMain(photo.id)} title="Hacer principal" className="rounded-lg bg-black/60 p-2 text-amber-300 disabled:opacity-40"><Star size={16} fill={photo.es_principal ? "currentColor" : "none"} /></button><button disabled={busy} onClick={() => setPhotoToRemove(photo)} title="Eliminar" className="rounded-lg bg-black/60 p-2 text-red-300"><Trash2 size={16} /></button></div></div>)}</div> : <div className="rounded-xl border border-dashed border-zinc-700 py-10 text-center text-zinc-500">Todavía no hay fotografías.</div>}
