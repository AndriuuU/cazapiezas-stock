...
export default function BarcodeScanner({ onScan, onClose, title = "Escanear referencia", description = "Busca la pieza mediante el código de barras.", manualPlaceholder = "Escanea o escribe la referencia", manualHint = "Los lectores físicos normalmente escriben el código y pulsan Enter automáticamente.", allowManualEntry = true }: { onScan: (value: string) => void; onClose: () => void; title?: string; description?: string; manualPlaceholder?: string; manualHint?: string; allowManualEntry?: boolean }) {
  ...
  return <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 p-3 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div role="dialog" aria-modal="true" aria-labelledby="barcode-scanner-title" className="w-full max-w-lg overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-950 shadow-2xl">
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3"><div><h2 id="barcode-scanner-title" className="flex items-center gap-2 font-bold text-white"><ScanBarcode className="text-cyan-400" /> {title}</h2><p className="mt-0.5 text-xs text-zinc-500">{description}</p></div><button onClick={onClose} title="Cerrar" className="rounded-lg p-2 text-zinc-400 hover:bg-zinc-800 hover:text-white"><X /></button></div>
      <div className="space-y-4 p-4">
        ...
        {allowManualEntry && <><div className="flex items-center gap-3 text-xs text-zinc-600"><span className="h-px flex-1 bg-zinc-800" />O USA UN LECTOR<span className="h-px flex-1 bg-zinc-800" /></div><form onSubmit={submitManual} className="space-y-2"><label className="flex items-center gap-2 text-sm font-semibold text-zinc-300"><Keyboard size={17} className="text-amber-400" /> Lector USB/Bluetooth o escritura manual</label><div className="flex gap-2"><input value={manualCode} onChange={(event) => setManualCode(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); finish(event.currentTarget.value); } }} placeholder={manualPlaceholder} className="min-w-0 flex-1 rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-3 font-mono text-white focus:border-amber-500 focus:outline-none" /><button disabled={!manualCode.trim()} className="rounded-xl bg-amber-500 px-4 font-bold text-zinc-950 hover:bg-amber-400 disabled:opacity-40">Buscar</button></div><p className="text-xs text-zinc-600">{manualHint}</p></form></>}
      </div>
    </div>
  </div>;
}
