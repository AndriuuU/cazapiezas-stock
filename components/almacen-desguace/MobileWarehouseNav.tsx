...
export default function MobileWarehouseNav() {
  ...
  return <>
    {open && <>
      ...
      <section aria-label="Más opciones del almacén" className="warehouse-mobile-nav-root warehouse-mobile-menu fixed rounded-3xl border border-zinc-700 p-3 shadow-2xl shadow-black" style={{ left: "0.75rem", right: "0.75rem", bottom: "calc(5.25rem + env(safe-area-inset-bottom))", zIndex: 60, backgroundColor: "#09090b" }}>
        <div className="mb-2 flex items-center justify-between px-2 py-1">
          <div><p className="font-black text-white">Más opciones</p><p className="text-xs text-zinc-500">Consulta y organiza el almacén</p></div>
    ...
    <nav aria-label="Navegación móvil del almacén" className="warehouse-mobile-nav-root fixed inset-x-0 bottom-0 border-t border-zinc-800 px-2 pt-2 shadow-[0_-12px_30px_rgba(0,0,0,0.65)]" style={{ zIndex: 50, paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))", backgroundColor: "#000" }}>
      <div className="mx-auto grid max-w-md items-end" style={{ gridTemplateColumns: "repeat(5, minmax(0, 1fr))" }}>
        <Link href="/almacen-desguace" className={`warehouse-bottom-nav-item ${piecesActive ? "text-amber-400" : "text-zinc-500"}`}><Warehouse className={piecesActive ? "-translate-y-0.5 scale-110" : ""} size={22} /><span>Piezas</span></Link>
        <Link href="/almacen-desguace/cajones" className={`warehouse-bottom-nav-item ${pathname.startsWith("/almacen-desguace/cajones") ? "text-cyan-300" : "text-zinc-500"}`}><Archive size={22} /><span>Cajones</span></Link>
        <Link href="/almacen-desguace/nueva" className="warehouse-bottom-nav-create" aria-label="Registrar nueva pieza"><span><Plus size={27} /></span><small>Nueva</small></Link>
        <Link href="/almacen-desguace/plano" className={`warehouse-bottom-nav-item ${pathname.startsWith("/almacen-desguace/plano") ? "text-cyan-300" : "text-zinc-500"}`}><MapPinned size={22} /><span>Plano</span></Link>
        <button type="button" aria-expanded={open} onClick={() => setOpen((current) => !current)} className={`warehouse-bottom-nav-item ${moreActive ? "text-cyan-300" : "text-zinc-500"}`}><MoreHorizontal className={open ? "-translate-y-0.5 scale-110" : ""} size={24} /><span>Más</span></button>
      </div>
    </nav>
