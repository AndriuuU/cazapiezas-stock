...
function ToolCardHeader({ tool, showDetails = false }: { tool: HerramientaComun; showDetails?: boolean }) {
  return <div className="sm:flex sm:gap-4 sm:p-4">
    ...
    <div className="min-w-0 flex-1 p-4 sm:p-0">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={tool.estado} /><ToolModeBadge tool={tool} /></div>
...
function MobileMore({ admin, missing, unlocated, archived, incidents, onMissing, onUnlocated, onCreate, onShelves, onLabels, onArchive, onIncidents }: { admin: boolean; missing: number; unlocated: number; archived: number; incidents: number; onMissing: () => void; onUnlocated: () => void; onCreate: () => void; onShelves: () => void; onLabels: () => void; onArchive: () => void; onIncidents: () => void }) {
...
function SearchLocationResults({ tools, shelves, onTool }: { tools: HerramientaComun[]; shelves: EstanteriaHerramientas[]; onTool: (tool: HerramientaComun) => void }) {
  return <div className="space-y-4"><section className="grid gap-3 sm:grid-cols-2">{tools.map((tool) => <button type="button" key={tool.id} onClick={() => onTool(tool)} aria-label={`Abrir ${tool.nombre}`} className={`rounded-xl border p-4 text-left transition active:scale-[0.99] ${tool.estado === "prestada" ? "border-amber-500/30 bg-amber-500/10" : "border-cyan-500/30 bg-cyan-500/10"}`}>
    <div className="flex flex-wrap items-start justify-between gap-2">
      ...
      <span className="flex flex-wrap justify-end gap-2"><StatusBadge status={tool.estado} /><ToolModeBadge tool={tool} /></span>
    </div>
...
function MovementBadge({ type }: { type: MovimientoHerramienta["tipo"] }) { if (type === "retirada") return <span className="rounded-full bg-amber-400 px-2.5 py-1 text-[10px] font-black text-zinc-950">RETIRÓ</span>; if (type === "devolucion") return <span className="rounded-full bg-emerald-400 px-2.5 py-1 text-[10px] font-black text-zinc-950">COLOCÓ</span>; if (type === "incidencia") return <span className="rounded-full bg-red-400 px-2.5 py-1 text-[10px] font-black text-zinc-950">INCIDENCIA</span>; if (type === "archivo") return <span className="rounded-full bg-zinc-400 px-2.5 py-1 text-[10px] font-black text-zinc-950">ARCHIVO</span>; return null; }
function movementLabel(item: MovimientoHerramienta) { if (item.tipo === "retirada") return `${item.empleado || "Empleado sin indicar"} retiró la herramienta`; if (item.tipo === "devolucion") return `${item.empleado || "Empleado sin indicar"} colocó la herramienta en su sitio`; if (item.tipo === "incidencia") return `${item.empleado || "Empleado sin indicar"} indicó: ${incidentLabel(item.incidencia_tipo)}`; return ({ alta: "Herramienta registrada", cambio_estado: "Estado actualizado", cambio_ubicacion: "Ubicación actualizada", edicion: "Datos de la herramienta editados", foto: "Fotografía actualizada", incidencia_resuelta: "Incidencia marcada como resuelta", archivo: "Herramienta archivada", restauracion: "Herramienta restaurada" })[item.tipo]; }
