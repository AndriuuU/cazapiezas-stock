...
export function ActionActorSelect({ currentUser, users, loading, value, onChange, label = "Quién realiza la acción", hint = "La operación quedará guardada a nombre de esta persona." }: { currentUser: AppUser | null; users: AppUser[]; loading: boolean; value: string; onChange: (id: string) => void; label?: string; hint?: string }) {
  ...
  return <label className="block"><span className="mb-2 flex items-center gap-2 text-sm font-bold text-zinc-300"><UserRound size={17} className="text-amber-300" />{label}</span><select required value={value} disabled={loading} onChange={(event) => onChange(event.target.value)} className="min-h-12 w-full rounded-xl border border-amber-500/40 bg-zinc-900 px-3 text-white outline-none focus:border-amber-400 disabled:opacity-50"><option value="">{loading ? "Cargando empleados…" : "Selecciona un empleado"}</option>{users.map((user) => <option key={user.id} value={user.id}>{user.nombre}{user.rol === "administrador" ? " · Administrador" : ""}</option>)}</select><span className="mt-1.5 block text-xs text-amber-200/70">{hint}</span></label>;
}
